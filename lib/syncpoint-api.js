...
function Syncpoint(config) {
    ...
    function bindToExistingControlDatabases(cb) {
        coux([config.host, config.users_db, "_design/syncpoint", "_view", "control_dbs"], 
            e(function(err, view) {
            view.rows.forEach(function(row) {
                userChannelControl.bindToControlDb(row.value, config)
            })
            cb(false)
        }))
    ...
    function createUserDDoc(cb) {
        var ddoc = require('./user_ddoc').ddoc;
