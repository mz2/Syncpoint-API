...
function Syncpoint(config) {
    ...
    function bindUserDb() {
      ...
      var control = docstate.control();
        ...
        //   console.log("_getState", doc.pairing_state)
        //   if (doc.pairing_state) cb(doc.pairing_state);
        // };
        // control._getType = function(doc, cb) {
        //   console.log("_getType", doc.pairing_type)
        ...
        coux.subscribeDb(config.host + "/" + config.users_db, function(change) {
          ...
          change.state = change.doc.pairing_state;
          change.type = change.doc.pairing_type;
          control.handle(change);
        });
    }
    ...
    //       coux.put([config.host, config.users_db, handshake._id], handshake, function(err, ok) {
