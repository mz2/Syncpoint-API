...
function setupControlOrSingleChannel(pairingUserDoc, userDoc, config, callback) {
  if (pairingUserDoc.pairing_mode == "single-channel") {
    setupSingleChannel(userDoc, config, callback)
  } else {
    setupControlDatabase(userDoc, config, callback)
  }
};

function setupSingleChannel(userDoc, config, callback) {
  function ensureChannelDatabase(userDoc) {
    coux([config.host, userDoc.channel_database], function(err, ok) {
        if (err) {
            console.log("create single channel", userDoc.channel_database, "for user", userDoc.name);
            coux.put([config.host, userDoc.channel_database], 
                e(secureChannelDatabase, [userDoc]));
        } else {
            secureChannelDatabase(userDoc);
        }
    })
  }
  
  function secureChannelDatabase(userDoc) {
    exports.addMemberToDatabase(userDoc._id, 
      [config.host, userDoc.channel_database], e(callback, [false, userDoc]));
  }
  
  if (!userDoc.channel_database) {
      coux([config.host, "_uuids"], e(function(err, data) {
          userDoc.channel_database = "channel-"+data.uuids[0];
          ensureChannelDatabase(userDoc)
      }))
  } else {
      ensureChannelDatabase(userDoc)
  }
...
exports.activateWebUser = function(webUserDoc, config) {
  setupControlOrSingleChannel(webUserDoc, webUserDoc, config, e(function(err, userDoc, isNew) {
...
exports.activatePairingUser = function(pairingUserDoc, userDoc, config) {
  ...
  // kick off the chain:
  setupControlOrSingleChannel(pairingUserDoc, userDoc, config, e(function(err, userDoc, isNew) {
