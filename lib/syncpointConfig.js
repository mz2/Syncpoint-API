...
function getConfig(config) {
  ...
  uri.pathname = "/"+config.admin_db+"/_design/console/pages/index.html";
  delete uri.auth;
...
function promptForConfig(cb) {
  ...
  prompt.get([{
    ...
    ensureAdminCreds(basicURL, data, function() {
      coux([basicURL, "_config", "couch_httpd_auth","authentication_db"], (function(err, authDb) {
        if (err) {
        ...
        prompt.get([{
          "name" : "admin_db",
          "message" : "Adminstration database",
          "default" : "sp_admin"
        }], function(err, config) {
